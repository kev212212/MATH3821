...
func1 = function(x){prod((1/sqrt(var(x))*sqrt(2*pi))*exp(-0.5*((x-mean(x))/sqrt(var(x)))))}
#Use CLT to approximate a normal distribution
for (k in 0:kmax) {
  bnp1 <- bn - gamma*RSS(bn)
  #finding score to use for convergence
  #Is this the correct score function because it never reaches below 0.00001
  if(norm_vec(bnp1-bn) <= 0.00001){
...
#about 5349 iterations were required 
'''
...
U.vec <- rep(1,2)
beta.vec <- c(0,0)
X <- as.matrix(cbind(rep(1,1000),x))
Y <- as.matrix(y)
iter = 0

while(norm_vec(U.vec) > 0.00001) {
  iter= iter +1
  eta.vec <- X %*% beta.vec
  Lambda.eta <- as.vector(plogis(eta.vec))
  W =  diag(Lambda.eta*(1-Lambda.eta))
  U.vec <- t(X) %*% (Y - Lambda.eta)
  I <- t(X) %*% W %*% X
  beta.vec <- beta.vec + solve(I) %*% U.vec
}
print(beta.vec)
print(iter)

#alpha from NR
beta.vec[1]+beta.vec[2]*mean(x)
#Using the closed form formula in c) we find alpha
mean(y)
#Finding beta
beta.vec[2]
sum(((y-mean(y))*(x-mean(x)))/(sum((x-mean(x))^2)))
#We can see that velues are wildly different
#3 iterations were required
#I cant get this part to work need to think of new method of getting hessian
'''
